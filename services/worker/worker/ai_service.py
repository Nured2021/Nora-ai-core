"""
NORA AI Service for Celery Worker — Phase 2

Standalone version of the AI service (no imports from services/api).
Shares the same OpenAI integration logic with graceful simulation fallback.
"""
...
def generate_plan(prompt: str) -> dict[str, Any]:
    """Generate a structured build plan via OpenAI, or return a simulation."""
    client = _get_client()
    if client:
        try:
...
def generate_code(plan: dict[str, Any]) -> list[dict[str, str]]:
    """Generate source files from a plan via OpenAI, or return stubs."""
    client = _get_client()
    if client:
        try:
            resp = client.chat.completions.create(
...
def _fallback_code(plan: dict[str, Any]) -> list[dict[str, str]]:
    ...
    return [{"path": path, "content": _stub_content(path, name)} for path in files]
